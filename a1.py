...
class GameBoard:
    ...
    # _characters:
    #   a list of characters on the game board.
    ...
    def place_character(self, c: Character) -> None:
        ...
        if isinstance(c, Player):
            self._player = c
        elif len(self.at(c.x, c.y)) == 1:
            g = self.at(c.x, c.y)[0]
            if isinstance(g, GarbageCan):
                if not g.locked and isinstance(c, Raccoon):
                    c.inside_can = True
        ...
        for character in self._characters:
            if isinstance(character, Raccoon):
                if character.check_trapped():
                    ...
                    return None
        self.ended = True
        return result_raccoon * 10 + self.adjacent_bin_score()
    ...
    def adjacent_bin_score(self) -> int:
        ...
        for i in range(self.height):
            for j in range(self.width):
                tile = self.at(j, i)
                if len(tile) == 1 and isinstance(tile[0], RecyclingBin) and \
                        self.helper_explore(j, i) != 0:
                    lst.append(self.helper_explore(j, i))
                elif len(tile) == 1 and isinstance(tile[0], RecyclingBin) and \
                        self.helper_explore(j, i) == 0:
                    lst.append(1)
                else:
                    pass
    ...
    def helper_explore(self, x: int, y: int, seen: list = None) -> int:

        """
        Return the biggest area that the Recyclybin neighbour can read reached
        without repeated by given x, y and a list of coordinate seen
        that have already seen before.

         If no recyclebin is connected return 0

        >>> b = GameBoard(3, 3)
        >>> _ = RecyclingBin(b, 1, 1)
        >>> _ = RecyclingBin(b, 0, 0)
        >>> _ = RecyclingBin(b, 2, 2)
        >>> b.helper_explore(0,0)
        0
        >>> _ = RecyclingBin(b, 2, 1)
        >>> b.helper_explore(2, 1)
        3
        """
        n = get_neighbours((x, y))
        copy_ = n.copy()
        seen = [] if seen is None else seen
        for coordinate in copy_:
            if not self.on_board(coordinate[0], coordinate[1]):
                n.remove(coordinate)
            elif len(self.at(coordinate[0], coordinate[1])) != 1:
                n.remove(coordinate)
            else:
                char = self.at(coordinate[0], coordinate[1])[0]
                if not isinstance(char, RecyclingBin):
                    n.remove(coordinate)
                elif (char.x, char.y) in seen:
                    n.remove(coordinate)
        seen.extend(n)
        if not n:
            return 0
        acc = len(n)
        for neighbour in n:
            acc += self.helper_explore(neighbour[0], neighbour[1], seen)
        return acc
...
class RecyclingBin(Character):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        x2 = self.y + direction[1]
        if not GameBoard.on_board(self.board, x1, x2):
            return False
        elif not GameBoard.at(self.board, x1, x2):
            self.x = x1
            self.y = x2
            return True
        elif isinstance(self.board.at(x1, x2)[0], RecyclingBin):
            if GameBoard.at(self.board, x1, x2)[0].move(direction):
                self.x = x1
                self.y = x2
                return True
            else:
...
class Player(TurnTaker):
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        x2 = self.y + direction[1]
        g = []
        if GameBoard.at(self.board, x1, x2):
            g = self.board.at(x1, x2)[0]
        if not GameBoard.on_board(self.board, x1, x2):
            return False
        elif not GameBoard.at(self.board, x1, x2):
            self.x = x1
            self.y = x2
            return True
        elif isinstance(g, GarbageCan):
            if g.locked:
                return False
            else:
                if len(self.board.at(x1, x2)) == 1:
                    g.locked = True
                    return True
                else:
                    return False
        elif isinstance(g, RecyclingBin):
            if GameBoard.at(self.board, x1, x2)[0].move(direction):
                self.x = x1
                self.y = x2
                return True
...
class Raccoon(TurnTaker):
    ...
    def check_trapped(self) -> bool:
        ...
        if self.inside_can:
            return False
        neighbour = get_neighbours((self.x, self.y))
        for i in neighbour:
            x = i[0]
            y = i[1]
            tile = self.board.at(x, y)

            if self.board.on_board(x, y) and len(tile) == 0:
                return False
            elif len(tile) == 1 and isinstance(tile[0], GarbageCan):
                return False
        return True
    ...
    def move(self, direction: Tuple[int, int]) -> bool:
        ...
        if self.board.at(x, y):
            g = self.board.at(x, y)[0]
        if self.board.on_board(x, y):
            if self.inside_can:
                return False
            elif not self.board.at(x, y):
                self.x = x
                self.y = y
                return True
            elif isinstance(g, GarbageCan):
    ...
    def take_turn(self) -> None:
        ...
        if not self.inside_can:
            for d in get_shuffled_directions():
                if self.move(d):
                    return
...
class SmartRaccoon(Raccoon):
    ...
    def take_turn(self) -> None:
        ...
        for d in DIRECTIONS:
            ...
            next_x = self.x + d[0]
            next_y = self.y + d[1]
            while self.board.on_board(next_x, next_y):
                if len(self.board.at(next_x, next_y)) == 0:
                    next_x += d[0]
                    next_y += d[1]
                    step += 1
                    continue
                elif len(self.board.at(next_x, next_y)) == 1 \
                        and isinstance(self.board.at(next_x, next_y)[0],
                                       GarbageCan):
                    direction_lst.append(d)
                    step_lst.append(step)
                    break
                elif len(self.board.at(next_x, next_y)) == 1 and \
                        isinstance(self.board.at(next_x, next_y)[0],
                                   (Raccoon, RecyclingBin)):
                    break
                elif len(self.board.at(next_x, next_y)) == 1 and \
                        isinstance(self.board.at(next_x, next_y)[0], Player):
                    next_x += d[0]
                    next_y += d[1]
                    step += 1
                    continue
                    ...
                    break
        if not direction_lst and not step_lst:
            Raccoon.take_turn(self)
            return None
        min_step = min(step_lst)
        for i in range(len(step_lst)):
            if step_lst[i] == min_step:
                self.move(direction_lst[i])
                break
